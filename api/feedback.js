...
・そこから読み取る深層（なぜ反応したか・何に惹かれるか）→ 推論：「〜なのかもしれません」「〜という表れでしょう」
全文を断言で埋めない。全文を推論で埋めない。自然に混ぜてリズムを作る。同じ語尾を3回以上連続させない。
...
【驚き・発見を必ず1つ入れる】
選んだ言葉・求めるもの・自由記述の "組み合わせ" から、本人がまだ言語化していない一歩踏み込んだ解釈を入れる。
例：「〜に反応しながら〜を求めるあなたは、実は〜なのかもしれません」

価値観3語は「〜できる人」「〜に動かされる人」など具体的な姿勢で。
例：「問いの熱に反応できる人」「整う前の場に惹かれる人」「他者の本気に乗れる人」

messageは10〜13行。改行は\\nで表現。短い断言と長めの推論文を混ぜる。型を感じさせず自然に、以下の流れで：
1. 今のあなたの状態（求めているもの・自由記述を根拠に断言）
2. なぜその経営層の言葉に反応したのか（言葉を引用し、経営層の人物像と結びつけて惹かれた理由を推論）
3. 選んだものと自由記述の組み合わせから見える、一歩踏み込んだ発見
4. この経営層との接点（希望する関わり方）が意味を持つ理由
5. 次の一歩への後押し

以下のJSON形式のみで返答（マークダウン・コードブロック・前置き一切不要）：
{"summary":"簡易フィードバック4〜5文。断言と推論を混ぜ、ユーザーに'見抜かれた'と感じさせる。完結させてよい。","values":["〜な人","〜できる人","〜に動かされる人"],"message":"詳細メッセージ10〜13行。改行は\\nで表現。","bridge":"この経営層との接点が持つ意味を1文で。"}`;
      const p = data.ceoProfile || {};
      const ceoContext = p.ceo ? `
─ 経営層の情報（診断の文脈に使う） ─
名前：${p.ceo}（${p.company || ''}）
キャッチフレーズ：${p.catchphrase || ''}
記事タイトル：${p.title || ''}
タグ：${(p.tags || []).join('、')}
会社紹介：${p.intro || ''}
